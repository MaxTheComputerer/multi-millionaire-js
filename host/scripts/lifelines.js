...
function select_5050()
{
	if(!locked && !used_5050)
	{
		...
		socket.emit('message', 'select_5050', {answer: answer, keep: keep});
		for (var i = 0; i < 4; i++) {
			if(i != answer && i != keep)
			{
...
function select_phone()
{
	if(!locked && !used_phone)
	{
		...
		$('.phone-choice-block').css('display','block');
...
function phone_real()
{
	...
	$('.phone-real-block').css('display','block');
	$('.phone-choice-block').css('display','none');
	socket.emit('message', 'select_phone');
}

function phone_clock_real()
{
	socket.emit('message', 'loadsound_clock');
	phone_clock_music = new Pizzicato.Sound('assets/sounds/lifelines/phone-clock.ogg', () => start_clock_real());
	...
	phone_clock_music.volume = 0;
	phone_clock_music.on('end', function(){
		question_music = playsound(sounds.questions.music[question_number], loop = true);
	});
}

function start_clock_real()
{
	socket.emit('message', 'start_clock');
	stopsoundnow(phone_start_music);
	$('#clock-real').attr('src', 'assets/images/clock.gif');
	phone_clock_music.play();
	socket.emit('message', 'playsound_clock');
	$('#phone-start-clock').removeClass('enabled');
	$('#phone-dismiss-real').addClass('enabled');
}
...
async function start_clock(response, answers, main_answer, secondary_answer)
{
	...
	var txt = response['text'].replaceAll('ANS1', answers[main_answer].toLowerCase()).replaceAll('ANS2', answers[secondary_answer].toLowerCase())
	socket.emit('message', 'phone_response', txt);
	$('#phone-text').text(txt);
...
{
	if($('#phone-dismiss').hasClass('enabled') || $('#phone-dismiss-real').hasClass('enabled'))
	{
		socket.emit('message', 'dismiss_phone');
		$('.phone-block').css('display','none');
		$('.phone-real-block').css('display','none');
		...
		if(phone_clock_music.playing)
		{
			...
			socket.emit('message', 'stopsound_clock');
		}
		unlock();
	}
}
...
async function select_audience()
{
	if(!locked && !used_audience)
	{
		used_audience = true;
		await lock();
		socket.emit('message', 'select_audience');
...
{
	socket.emit('message', 'draw_audience_graph', votes);
	...
	stopsoundnow(audience_vote_music);
	display_background();
...
function dismiss_audience()
{
	...
	{
		socket.emit('message', 'dismiss_audience');
